...
loadGSTimesheets = function () {
    ...
    GSTimesheets.prototype.set = function (username, date, params) {
        ...
        _.extend(row, _.pick(params, 'signIn', 'breakIn', 'breakOut', 'signOut', 'note'));

        var sheet = this._getSheet(username);
        var rowNo = this._getRowNo(username, date);

        var data = [DateUtils.toDate(date), row.signIn, row.breakIn, row.breakOut, row.signOut, row.note].map(function (v) {
            return v == null ? '' : v;
        });
